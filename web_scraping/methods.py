...
def get_match_dates(team_name, year):
    ...
    for k in range(len(team_schedule)):
        date = ""
        # print(len(team_schedule[k].find_all("span")))
        if len(team_schedule[k].find_all("span")) != 0:
            for i in team_schedule[k].find_all("span"):
                # print(i.has_attr("class") and any("team-schedule__game-date" in j for j in i["class"]))
                if i.has_attr("class") and any("team-schedule__game-date" in j for j in i["class"]):
    ...
    for k in range(len(team_schedule)):
        ...
        if len(team_schedule[k].find_all("span")) != 0:
            for i in team_schedule[k].find_all("span"):
                ...
                if i.has_attr("class") and len(i["class"]) > 0 and ("team-schedule__info" in i["class"]
...
def get_dataset(team, year, team_pairs):
    ...
    if team_schedule is not None:
        ...
        dates = get_match_dates(team, year)
        ...
        for table in stats_tables:
            ...
            match_stats["Location"] = locations[match_no] if len(locations) > match_no else "-"
            match_stats["Date"] = dates[match_no] if len(dates) > match_no else "-"
            match_stats["Year"] = year
            if match_stats['Team1_Team2'] not in team_pairs:
                team_pairs.add(match_stats['Team1_Team2'] + "_" + match_stats["Date"])
